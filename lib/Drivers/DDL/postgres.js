...
function buildColumnDefinition(driver, table, name, prop) {
	var def;	
	if (typeof prop.coldef !== 'undefined' && typeof prop.coldef.postgres !== 'undefined') {
		def = def = driver.query.escapeId(name) + " " + prop.coldef.postgres;		
	} else {
		switch (prop.type) {
			case "text":
			...
			case "number":
				if (prop.rational === false) {
					...
					if( hasCustomType ) {
						def = driver.query.escapeId(name) + " " + prop.coldef.postgres.type;
						if ( typeof prop.coldef.postgres.precision !== 'undefined') {
							var p = [];
							p.push( driver.query.escapeVal( prop.coldef.postgres.precision ) );
							if (typeof prop.coldef.postgres.scale !== 'undefined') {
								p.push( driver.query.escapeVal( prop.coldef.postgres.scale ) );
							}
							def += "(" + p.join(",") + ")";
						}
					} else {
