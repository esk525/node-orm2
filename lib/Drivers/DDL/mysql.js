...
function buildColumnDefinition(driver, name, prop) {
	var def;
	if (typeof prop.coldef !== 'undefined' && typeof prop.coldef.mysql !== 'undefined') {
		def = driver.query.escapeId(name) + " " + prop.coldef.mysql;		
	} else {
		switch (prop.type) {
			case "text":
			...
			case "number":
				if (prop.rational === false) {
					...
					if( hasCustomType ) {
						def = driver.query.escapeId(name) + " " + prop.coldef.mysql.type;
						if ( typeof prop.coldef.mysql.precision !== 'undefined') {
							var p = [];
							p.push( driver.query.escapeVal( prop.coldef.mysql.precision ) );
							if (typeof prop.coldef.mysql.scale !== 'undefined') {
								p.push( driver.query.escapeVal( prop.coldef.mysql.scale ) );
							}
							def += "(" + p.join(",") + ")";
						}
					} else {
